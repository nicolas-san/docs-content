...
The Message filters template feature is currently in **beta** status.

</Message>

The Message filters accept templating with some metadata associated with the Hub and Device to be used in the filter. This allows to use the same filter on multiple devices and have them filter topics with regards to their own name or IDs.
...
To use them in your message filters use double curly brackets around the element you want to use.
For example:
```
...
Issues when parsing the template will be reported through [Hub events](https://www.scaleway.com/en/docs/scaleway-iothub-events/) as warnings, in that case the filter may be dropped from the list.
